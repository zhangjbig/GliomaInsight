...
png( paste0(datasets, "_", methods, "_OS_KM.png"), width = 471, height = 461)
...
score_final_sample <- data.frame(score_final)
score_final_sample$group <- 1
score_final_sample$group[which(score_final_sample$score > cutoff_RF_OS)] <- 2
print(score_final_sample$group)

#存储分组，只输出本分组
i = score_final_sample$group
print(score_final_sample$group)
...
if(i==1){
  ...
  p <- ggsurvplot(fit,
                  ...
                  risk.table.col = "group", # Change risk table color by groups
                  ggtheme = theme_classic(), # Change ggplot2 theme
                  palette = c("transparent","#66CCCC" )
  )
}
png( paste0(datasets, "_", methods, "_Sample_KM.png"), width = 471, height = 461)
print(p$plot, newpage = FALSE)
dev.off()
